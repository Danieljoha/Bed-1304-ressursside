---
title: '[< Forrige](2%20-%20funksjoner.ipynb)     |     [Neste >](4%20-%20lister%2C%20oppslag%20og%20numpy.ipynb)'
jupyter: python3
---



<span style="color:#9bb1d4;font-size:30px;font-weight:900"> 7.1 - matplotlib </span>  
...
# Det ser ut til at tilbud og etterspørsel er lik rundt 65 enheter, til en pris litt under 20.

### Oppgave 1.2 ###
# Obs, dette skal vi lære å gjøre i neste forelesning...
### Dette kan vi løse på en mer sofistikert og analytisk måte også!
import sympy as sp 
...
plt.plot(df['BNP_per_capita'], df['Befolkning'], 'r')
...
plt.scatter(df['Utdanning'], df['Befolkning'])
...
plt.plot(x, profit(5,x), label='Fortjeneste gitt mengde 5')
plt.plot(x, profit(x,10), label='Fortjeneste gitt pris 10')
...
```{python}
df_sort = df.sort_values('Utdanning')
bnp_per_capita=np.array(df_sort['BNP_per_capita'])
utdanning=np.array(df_sort['Utdanning'])

plt.scatter( utdanning, bnp_per_capita)
plt.plot(utdanning, np.exp(utdanning*0.7+2) )
...
plt.scatter( np.array(df['Utdanning']), bnp_per_capita)
plt.plot(utdanning, np.exp(utdanning*0.7+2) )
...
# parametere
start_price = 70  # Startpris på ost i første år
growth_rate = 0.02  # 2% vekst per år
years = np.arange(2003, 2013+1)  # Fra år 2003 til 2013
randomness = 0.05  # +/- 5% tilfeldig variasjon hvert år

np.random.seed(123)

# Generer ost-prisene med vekst og tilfeldig variasjon
cheese_prices = [start_price]
for year in years[1:]:
    last_price = cheese_prices[-1]
    # Øk prisen med 2% og legg til tilfeldig variasjon
    new_price = last_price * (1 + growth_rate) * (1 + np.random.uniform(-randomness, randomness))
    cheese_prices.append(new_price)

# Plot prisen for hvert år
plt.plot(years, cheese_prices, marker='o', linestyle='-', color='darkblue')

# Legg til tittel og akse-labels
plt.title("Cheese Price Over the Years (with 2% annual increase and randomness)")
plt.xlabel("Year")
