---
title: '[< Forrige](4%20-%20lister%2C%20oppslag%20og%20numpy.ipynb)     |     [Neste >](6%20-%20pandas%2C%20filtrering%2C%20logikk%20og%20betingelser.ipynb)'
jupyter: python3
---
...
# Løs likningssettet
løsninger = sp.solve([eq1, eq2], (x, y), dict=True)

# Print resultatene
for sol in løsninger:
...
# Definer funksjonene f(x) og g(x)
f = sp.Function('f')(x)
g = sp.Function('g')(x)
...
# Deriver produktet
derivert = sp.diff(produkt, x)

print("d/dx [f(x)*g(x)] =", derivert)
```
...
# f(y) = sqrt(y)
# g(x) = 2*x**2 + 5
f1 = sp.sqrt(sp.symbols('y'))
g1 = 2*x**2 + 5
h1_prime = f_deriv_chain(f1, g1, x)
print("Test 1: h'(x) =", h1_prime)
```
...
f2 = sp.symbols('y')**5
g2 = x**3 + 6*x
h2_prime = f_deriv_chain(f2, g2, x)
print("Test 2: h'(x) =", h2_prime)
```
